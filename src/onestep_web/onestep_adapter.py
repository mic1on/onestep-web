...
def build_runtime_app(
    ...
    *,
    ...
    try:
        ...
        raise PipelineCompileError(f"pipeline could not be loaded by OneStep: {exc}") from exc

    @app.on_event
    async def write_task_event(event: Any) -> None:
        message = f"task {event.kind.value}"
        if event.failure is not None:
            message = f"{message}: {event.failure.message}"
        await log(event.kind.value, event.task, message)
...
def _install_handler_module(module_name: str, handlers: Mapping[str, str]) -> None:
    ...
    for node_id, code in handlers.items():
        ...
        try:
            exec(compile(code, f"<onestep-web:{node_id}>", "exec"), namespace)
        except Exception as exc:
            raise PipelineCompileError(f"handler node {node_id} could not be loaded: {exc}") from exc
